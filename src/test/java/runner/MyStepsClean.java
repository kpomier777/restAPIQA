...
public class MyStepsClean {
    Response response;
    public static RequestInformation globalResponse;
    ...
    @Then("I send a {} request to {} using the token {} with the json")
    public void iSendAPOSTRequestToPATHUsingATokenWithTheJson(String method, String path, String token, String body) {
        request.setAuthType(Config.TOKEN);
        request.setAuthValue(data.get(token));
        request.setUrl(replaceAllData(path));
        request.setBody(replaceAllData(body));
